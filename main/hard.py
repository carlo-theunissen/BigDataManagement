...
def get_flat_map(lhs_size, candidate_FDs):
    if(lhs_size == 1):
        return lambda row: [((i, row[fd[0]]), (row[fd[1]], True)) for i, fd in candidate_FDs]
    if(lhs_size == 2):
        return lambda row: [((i, row[fd[0]], row[fd[1]]), (row[fd[2]], True)) for i, fd in candidate_FDs]
    if(lhs_size == 3):
        return lambda row: [((i, row[fd[0]], row[fd[1]], row[fd[2]]), (row[fd[3]], True)) for i, fd in candidate_FDs]
